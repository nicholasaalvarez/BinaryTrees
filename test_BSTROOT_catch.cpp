...
// Create BST Obj
cop3530::BSTROOT<int, char, compare_Kint, is_equals_Kint> bst;
...
TEST_CASE( "BST Duplicate Key Test" ) {
...
TEST_CASE( "BST Remove Test" ) {
    ...
    // This Test will used the  previous BST
    // Since this is Root Insertion lets remove
    // node with Key=25 first since it is the root node.
    REQUIRE( bst.size()         == 14   );  // 14 Nodes
    REQUIRE( bst.contains(25)  == true  );
    bst.remove(25);
    REQUIRE( bst.contains(25)  == false );
    REQUIRE( bst.size()        == 13    );  // 13 Nodes
    REQUIRE( bst.height()      == 6     );
    ...
    // look up 59
    // lets check that key=58 is still in BST
...
TEST_CASE( "BST Insertion Algorithm Test" ) {
    ...
    // It will insert 100 nodes all in increasing fashion
    //
    // For ROOT Tree = LEAF TREE
    //   ... if keys entered in ascending order worst case N
    //   ... Mean is 6.22 Log N = 6.22 X 6.64 = 41
    //   ... if keys entered randomly  1.38 Log N
    // BST Leaf -> Height 100-1 = 99
    // FOR BSTRADN: Random Insertion 2Log2N  (log2 100 )=6.64
    // FOR BSTAVL:  Perfectly balanced tree log2N
    //
    // Height of a perfectly Random Built tree is O(logN)
    // Random Insertion Worst Mean=2Lg(N) = 2(log2 100 )=2(6.64) = 13.24  100  Nodes
    // Random Insertion Worst Mean=2Lg(N) = 2(log2 1000)=2(9.64) = 18.65  1000 Nodes
    ...
    REQUIRE( bst.height()   ==  999  );  // BSTAVL=9  BSTRAND=170   BSTLEAF/ROOT=999 (N-1)
    REQUIRE( bst.balance()  ==  999  );  // BSTAVL=0  BSTRAND=170   BSTLEAF/ROOT=999 (N-1)
    std::cout<<"BSTROOT: Inserting Ascending Keys 0-1000 Balance= "<<bst.balance()<<" Height ="<<bst.height()<<std::endl;
    ...
    REQUIRE( bst.size()    <=  1000  );
    REQUIRE( bst.height()  <=  999   );  // BSTAVL=11  BSTRAND=21  BSTLEAF/BSROOT=
    REQUIRE( bst.balance() <=  999   );  // BSTAVL=1   BSTRAND=3   BSTLEAF/BSROOT=
    std::cout<<"BSTROOT: Inserting Random    Keys 0-1000 Balance=  "<<bst.balance()<<" Height ="<<bst.height()<<std::endl;
    std::cout<<"BSTROOT: SIZE =  "<<bst.size()<<std::endl;
...
TEST_CASE( "BST Copy Constructor/Assignment Test" ) {
    ...
    cop3530::BSTROOT<int, char, compare_Kint, is_equals_Kint> dst(bst);
    REQUIRE( dst.size()  == 12 );
